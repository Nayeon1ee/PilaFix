				...
				<c:choose>
				    <c:when test="${question.qsAnswerYn}">
				        ...
				        <button type="button" class="btn btn-primary mb-3" 
				                style="font-size: 10px; background: dimgray; border-color: dimgray;"
				                onclick="window.location.href='상세페이지URL?qsNumber=${question.qsNumber}'">
				            답변완료
				        </button>
				    </c:when>
				    <c:otherwise>
				        <!-- 답변대기 상태인 경우 -->
				        <button type="button" class="btn btn-primary mb-3" 
				                style="font-size: 10px; background: red; border-color: red;"
				                onclick="window.location.href='상세페이지URL?qsNumber=${question.qsNumber}'">
				            답변대기
				        </button>
				    </c:otherwise>
