...
<body>
	...
	<%--<script src="assets/js/main.js"></script>--%>
	 <script src="${pageContext.request.contextPath }/resources/js/notice_1.js"></script> 
	
	<script>
	<%-- export default{
		data(){
			return{
				events:[] // 수신된 이벤트 저장할 배열 
			};
		},
		mounted(){
		...
		methods:{
			setupEventSource(){
				const eventSource = new EventSource('http://localhost:8080/pilafix/sse/stream'); //SSE 연결 생성 
			...
			handleEvent(event){
				...
				this.events.push(eventData); //이벤트 데이터 배열에 추가 
	...
	const eventSource = new EventSource("sse/stream.do");
